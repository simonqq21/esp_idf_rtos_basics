...
/*
notify the ADC_sampling_task to read an ADC sample
*/
...
*/
static void ADC_sampling_task(void *param)
{
    uint32_t arg_val;
    while (1)
    {
        if (xQueueReceive(adc_evt_queue, &arg_val, portMAX_DELAY))
        {
            ...
            ESP_ERROR_CHECK(adc_oneshot_read(adc_handle, ADC_INPUT_CHAN, write_to));
...
// hardware timer ISR that samples ADC pin every 100 ms.

/*
task A averages previously collected 10 samples and writes it to a global floating point variable.
*/
static void averaging_task(void *param)
{
    while (1)
    {
...
/*
task V echoes any character received from the UART back to the same UART.
...
void app_main(void)
{
    ...
    xTaskCreatePinnedToCore(ADC_sampling_task,
                            "ADC sampling task",
                            1024,
                            NULL,
                            1,
    ...
    xTaskCreatePinnedToCore(averaging_task,
                            "averaging task",
                            1024,
                            NULL,
                            1,
